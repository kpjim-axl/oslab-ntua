...
void server(int port){
	...
	for(fd=0; fd<FD_SETSIZE; fd++){
		...
		if (fd == 0){
		     ...
		     * 
		     * Τέλος ο server μπορεί να εξυπηρετήσει και αιτήσεις των client. Ο τρόπος εξηγήται παρακάτω
...
// 				fflush(stdin); /* ? */
			...
			default:
			{
			    ...
			    sprintf(msg, "server: %s", kb_msg+1);	// mallon thelei kb_msg + 1
			    for (i=0; i<clients; i++)
				if (fd_array[i] == to){
				    write(to, msg, strlen(msg));
				    break;
				}
			    if (i==clients)
			...
			write(newsd, msg, strlen(msg));		// FIXME: insist_write
		    }
		    else {
