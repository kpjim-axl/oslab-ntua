                                   ...
                                   struct crypto_vq_buffer *buf)
{
	...
	case VIRTIO_CRYPTO_DEVICE_HOST_OPEN:
		...
		spin_lock_irq(&crdev->fd_lock);
		crdev->fd = cpkt->value;
		spin_unlock_irq(&crdev->fd_lock);
...
static int __devinit virtcons_probe(struct virtio_device *vdev)
{
	...
	/* Do i need any lock? */
	/* ? */
	spin_lock_init(&crdev->c_lock);
	spin_lock_init(&crdev->inbuf_lock);
	spin_lock_init(&crdev->fd_lock);
	...
	nr_added_bufs = fill_queue(crdev->ivq, &crdev->inbuf_lock);
	if (!nr_added_bufs) {
	...
	nr_added_bufs = fill_queue(crdev->c_ivq, &crdev->c_lock);
	if (!nr_added_bufs) {
