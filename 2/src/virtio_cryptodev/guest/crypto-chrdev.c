...
/****************************************
 * Implementation of file operations for
 * the Crypto character device.
 * TODO: add extra info so host can send
 *       us errno, etc..
 ****************************************/
...
static int crypto_chrdev_open(struct inode *inode, struct file *filp)
{
	...
	spin_lock_irqsave(&crdev->fd_lock, flags);
	if (crdev->fd >= 0) {
		spin_unlock_irqrestore(&crdev->fd_lock, flags);
		debug("Too many open files. fd=%d\n", crdev->fd);
		ret = -EMFILE;
		goto out;
	}
	
	/* kpjim: when I give the lock away, an other process may check crdev->fd..
	 * 	  if I haven't got the host_fd yet, the process will use the same
	 * 	  device.. So we're going to set crdev->fd temporary to 0 to stop it.
	 */
	crdev->fd = 0;
	spin_unlock_irqrestore(&crdev->fd_lock, flags);
	...
	/* Sleep here until we get the actual fd from the Host. */
